 ...
 * identifying the cursor. The cursor contains all cursor events of the
 * correspinding cursor (or finger when using multi-touch). Also, the cursor
 * allows the input processors to negotiate who has priority to use (lock) this
 * cursor.
 * 
...
//		events = new LinkedList<AbstractCursorInputEvt>();
		...
		lockingProcessorsToPriority = new TreeMap<AbstractCursorProcessor, Float>(
				new Comparator<AbstractCursorProcessor>() {
					...
					public int compare(AbstractCursorProcessor o1, AbstractCursorProcessor o2) {
						if (o1.getLockPriority() < o2.getLockPriority()) {
							...
							if (!o1.equals(o2) && o1.getLockPriority() == o2.getLockPriority()) {
								return -1;
							}
							return 0;
						}
		...
		registeredProcessorsToPriority = new TreeMap<AbstractCursorProcessor, Float>(
				new Comparator<AbstractCursorProcessor>() {
					 @Override
					public int compare(AbstractCursorProcessor o1, AbstractCursorProcessor o2) {
						if (o1.getLockPriority() < o2.getLockPriority()) {
	...
	/**
	 * Locks this cursor with the specified processor if the processors lock
	 * priority is higher or equal than the current lock priority of this cursor.
	 * 
...
//		}
		...
		if (currentLockPriority == ia.getLockPriority()) {
			...
			logger.debug("Cursor: " + this.getId()
					+ " LOCKED sucessfully, dont send lock signal because cursor was already locked by same priority ("
					+ currentLockPriority + ")");
			return true;
		} else if (currentLockPriority < ia.getLockPriority()) {
			lockingProcessorsToPriority.put(ia, ia.getLockPriority());
			...
			logger.debug("Cursor: " + this.getId()
					+ " LOCKED sucessfully, send lock signal - Cursor priority was lower " + "(" + currentLockPriority
					+ ")" + " than the gesture priority (" + ia.getLockPriority() + ")");
			// Send only to ones lower than this priority
			cursorLockedByHigherPriorityGesture(ia, ia.getLockPriority());

			// FIXME TEST - remove the lower priority gestures from locking this cursor
			// AFTER having sent the lockLost signal to these lower
			// priority processors! -> (in cursorLocked() , processor.getLockedCursors()
			// will still return this cursor! - check processor behaviour!
			// FIXME MTInputPositionEvtEST - ONLY KEEPING MTInputPositionEvtHE HIGHEST
			// PRIORITY ANALYZERS - just keep an array with the current highest priority
			// analyzers?
...
//			lockSeekingAnalyzersToPriority.put(ia, ia.getLockPriority()); //TODO REMOVE?
			logger.debug("Cursor: " + this.getId() + " LOCKED UN-sucessfully, send no lock signal - Cursor priority "
					+ "(" + currentLockPriority + ")" + " higher than the gesture priority (" + ia.getLockPriority()
					+ ")");
			return false;
		}
	}
...
//		return lockSeekingProcessorsToPriority.containsKey(cp);
		for (AbstractCursorProcessor abstractCursorProcessor : lockingProcessorsToPriority.keySet()) {
			if (abstractCursorProcessor.equals(cp) || abstractCursorProcessor.getLockPriority() == -1 // FIXME TEST!
			) {
				return true;
			}
		}
	...
	private void cursorLockedByHigherPriorityGesture(AbstractCursorProcessor ia, float gesturePriority) {
		if (!registeredProcessorsToPriority.isEmpty()) {
			...
			// FIXME use headMap -> inclusive = false? so it doesent send cursorLocked to
			// some with same priority
			SortedMap<AbstractCursorProcessor, Float> lesserPriorityGestureMap = registeredProcessorsToPriority
					.headMap(ia); // get analyzers with strictly lower priority than the locking one
			...
			for (AbstractCursorProcessor processor : lesserPriorityGestureKeys) {
				// Only send lock signal to the processors whos priority is lower than the
				// current locking cursor priority
				AbstractCursorProcessor a = processor;
				logger.debug("Cursor: " + this.getId() + " Sending cursor LOCKED signal to: " + a.getName());

				// FIXME the lock is already lost here? -it was like this, yes -> change so we
				// can use
				// if (this.getLockedCursors.contains(inputCursor)) ... in the processors'
				// cursorLocked() method to check if
				// the cursor was used in the processor
				// TODO check if we get concurrent modification exception if the processors
				// would call unlock() while we iterate here!!!
				if (this.isLockedBy(processor) && processor.getLockPriority() != -1) {// FIXME test - only send lockLost
																						// to processors who actually
																						// had the cursor locked
					// FIXME how to treat processor priority -1 ? we want to return true at
					// isLockedBy but we don't want to send cursorLocked to them!
...
//                processor.cursorLocked(this, ia);
	...
	/**
	 * Input processors should call this when new input has started to be able to
	 * use the cursor locking mechanisms.
	 * 
	 * @param ia the ia
	 */
...
//		}
	...
	/**
	 * Unlocks this cursor from the specified processor. If the priority by which
	 * this cursor is locked changes by that, the <code>cursorUnlocked</code> method
	 * is invoked on processors with a lower priority who by that get a chance to
	 * lock this cursor again.
	 * 
	 * @param ia the AbstractCursorProcessor
	 */
...
//		}
		...
		// should actually not occur because we should only call release when we have a
		// lock on the cursor
		...
		// Only send released signal if the priority really was lowered by releasing
		// (there can be more than 1 lock with the same lock priority)
		if (beforeLockPriority > afterRemoveLockPriority) {
			if (!registeredProcessorsToPriority.isEmpty()) {
				// Get strictly smaller priority gestures than the one releasing, so that the
				// ones with same priority dont get a signal
				SortedMap<AbstractCursorProcessor, Float> lesserPriorityGestureMap = registeredProcessorsToPriority
						.headMap(registeredProcessorsToPriority.lastKey());
//					SortedMap<IInputAnalyzer, Integer> lesserPriorityGestureMap = watchingAnalyzersToPriority.headMap(ia);
				Set<AbstractCursorProcessor> lesserPriorityGestureKeys = lesserPriorityGestureMap.keySet();
				for (AbstractCursorProcessor processor : lesserPriorityGestureKeys) {
					// Only send released signal to the analyzers whos priority is higher than the
					// current cursor priority
					// the current highest priority of the cursor can change when released is called
					// on a gesture that successfully
					// locks this cursor, so check each loop iteration
					if (processor.getLockPriority() < unlockingGesturePriority // Only call on gestures with a lower
																				// priority than the one releasing the
																				// lock
							&& this.getCurrentLockPriority() <= processor.getLockPriority() // only call unLocked on
																							// processors with a lower
																							// or equal lockpriority
//                                && this.isLockedBy(processor) //FIXME TEST
					) {
//                            processor.cursorUnlocked(this);
						processor.cursorFreed(this);
						// FIXME funktioniert das, wenn bei getLock in anderer geste wieder was in die
						// liste geadded wird etc?
					}
				}
			}
		}
...
//        }
		if (events.size() > EVENT_HISTORY_DEPTH) {
			// Don't remove the first event in case a user requests the start position of
			// the cursor.
			events.remove(1);
...
//		}
	...
	/**
	 * Gets the target layer of this cursor. (The layer the cursor started on) or
	 * null if it has no target or the cursor has no input events yet.
	 *
	 * @return the target
	 */
...
//		return returnEvent;
		...
		for (int i = 0; i < allEvents.size(); i++) {
			...
			if (event.equals(te) && (allEvents.size() >= 2) && i - 1 > 0 && allEvents.get(i - 1) != null) {
	...
	/*
	 * //TODO make velocity time based? //FIXME EXPERIMENTAL! public float
	 * getVelocityX(){ if (this.events.isEmpty() || this.events.size() < 2) return
	 * 0;
	 * 
	 * 
	 ...
	 * 
	 * // System.out.println("Pos: " + pos); float mouseNormX = pos.x * invWidth;
	 * float mouseVelX = (pos.x - prevPos.x) * invWidth;
	 * System.out.println("Mouse vel X: " + mouseVelX + " mouseNormX:" +
	 * mouseNormX); return mouseVelX; }
	 * 
	 * //FIXME EXPERIMENTAL! public float getVelocityY(){ if (this.events.isEmpty()
	 * || this.events.size() < 2) return 0;
	 * 
	 * AbstractCursorInputEvt posEvt = events.get(events.size()-1);
	 * AbstractCursorInputEvt prev = events.get(events.size()-2);
	 * 
	 ...
	 * 
	 * float mouseNormY = pos.y * invHeight; float mouseVelY = (pos.y - prevPos.y) *
	 * invHeight; System.out.println("Mouse vel Y: " + mouseVelY + " mouseNormY:" +
	...
	/**
	 * Calculates and returns the velocity vector. The calculation takes the events
	 * of the last milliseconds into account. The calculation is not physically
	 * correct but provides a good vector to use as inertia.
	 * 
